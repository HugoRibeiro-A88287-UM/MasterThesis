...
that require a designated area which involves costs and space. The second reason is time. In a competitive industry, the first to have the 
...
Picture a scenario with two processes. One process writes values in the memory and the other reads. Each operation takes five nanoseconds, 
...
of the \gls{rwth} Aachen University, which will be the reference work for this dissertation. In conclusion, other simulators will be explored 
...
In the end, it was concluded the speedup of the simulation as a function of the quantum behaves similarly to a sigmoidal or an "S"-shaped curve, 
...
it requires external modules to complement itself, for instance, it can not perform cycle-accurate simulations, and most of the modules/\glspl{ip} are 
...
currently not undergoing active development but is still widely utilized. The following image presents a features comparison between the 
previous simulators. 
...
is when a person is tasked to sort apples, where the color can be used to distinguish the apples and perform the sort operation. 
