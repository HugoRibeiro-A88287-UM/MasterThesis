...
a substantial processor overhead \cite{LinuxDeviceDrivers}. \glsxtrfull{asic}, as the name suggests, is used for specific applications where 
...
the \glsxtrfull{td} technique, since both can use it in order to obtain more performance. Moreover, all further context will be related to 
...
exchange, there is a \glsxtrfull{cs}, where happens a synchronization with the rest of the system. All variables are updated, thus, when a 
...
library for system and hardware design, to improve the performance is \glsxtrfull{td} \cite{systemC}. \gls{td} is a technique where individual 
