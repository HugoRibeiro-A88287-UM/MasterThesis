...
In this chapter, they will be described and explained in detail. These algorithms are called: ADALINE, increment, \gls{pc}, and 
...
previous scenario. In the last one, the final algorithm will be exposed, comparing its results with the static version already present in the
par-gem5.
...
Since the main objective is to evaluate the algorithms' execution, it is necessary to know exactly what type of tests these will be facing. Some 
workloads focus more on performance, while others prioritize inter-process communications, for example. Moreover, the host and target systems
configurations must also be provided, as different configurations may result in different performance results. Thereby, 
the \autoref{tab:hostSystemConfig} and \autoref{tab:targetSystemConfig} show the utilized setup for both host and target systems, respectively, 
for all executed benchmarks.
...
In the current state of par-gem5, the quantum attribution is done as presented in the \autoref{fig_GlobalSyncEventStatic}. The simulator enters a synchronization state 
...
To integrate the dynamic quantum two changes were done on the original version of par-gem5. The first and most important one is the creation 
of the \textit{QuantumMode}, and the other is the inclusion of more simulation statistics, particularly regarding the dynamic algorithm. 
...
alteration was required on the \textit{QuantumGlobalSyncEvent} to be capable of identifying, in the synchronization process, which version 
is being used. 
...
In the context of this dissertation, it will be used two \gls{cpu} benchmarks, the bare-metal bubble sort and the \gls{npb}.
\newline
...
The ADALINE's algorithm output, that is, the filtered quantum, will be always lower than the source. Using this new quantum implies there will 
...
the threshold, making it harder to increase the incValue. Moreover, the increment value only changes (increases or decreases) if it 
fulfills the condition for N consecutive times. The later image shows the algorithm's flowchart. 
...
The ADA refers to the first results, while A-I is related to the new configuration. In general, the dynamic approach with the 
...
Unfortunately, the same did not occur in terms of accuracy. In all cases, just with a few exceptions, 
...
By evaluating the \gls{pc}, it is possible to understand if the processing of one memory address can result in a postponed event (PPaddr), and so 
...
in the A-I version hence, the previously mentioned cases that passed the 5\% are now within the limit. Only the NPB FT with 32
...
It can be concluded the \gls{pc} algorithm presence allowed the quantum to immediately reduce its value. It also can be identified when 
...
\begin{figure}[h!]
	\centering
 	\includegraphics[width=0.45\linewidth]{Images/ADA_and_REP.png}
...
To calculate the quantum, the last task uses an iteration technique, as shown in detail in the \autoref{fig_Repetition_flowchart_quantum}. 
...
with the minimum value, which is the clock period, and increments that value in each iteration. 
...
After the development and testing of all aforementioned algorithms, it was determined the better approach is the combination of the ADALINE, 
...
as illustrated in the following flowchart. The static version was not removed because with a-priory information about the benchmark, 
...
The next graphs present a comparison between the final algorithm and the static mode with one microsecond synchronization time. 
...
\caption{Final algorithm and static mode comparison}
