...
In this chapter, they will be described and explained in detail. These algorithms are called: ADALINE, increment, \glsxtrfull{pc}, and 
...
In the current state of par-gem5, the quantum attribution is done as presented in the latter image. The simulator enters a synchronization state 
...
The \glsxtrfull{npb} \cite{bailey1994parallel} is a group of a small set of programs designed to help the performance evaluation of parallel 
...
\begin{itemize}
    ...
    \item \textbf{Fast Fourier Transform (FT):} Solves a three-dimensional partial differential equation using \glsxtrfullpl{fft}. 
...
of a \glsxtrfull{anc} algorithm. A generic scheme can be found in the image below.
...
filter was the implementation of a \glsxtrfull{tdl}. Its operating method is described in the figure below. 
...
%Alterar estas comparações, usar os valores todos
...
\begin{figure}[h!]
	\centering
 	\includegraphics[width=0.75\linewidth]{Images/Repetition_flowchart.png}
...
did not happen, in particular, NPB BT, LU, and SP with 32 and 64 simulated cores, the performance difference was huge. When calculating the 
mean of performance gain and comparing it with the static version, if these are considered, the result would be -9.1\%. Without the previous 
