...
and explained in detail. Regarding the overall simulator, as presented in the \autoref{fig_HighLevelPar_gem5OpMode}, 
these will operate on the synchronization process, more precisely in the quantum definition. 
...
All event queues are running independently from each other until they reach the barrier, which is the defined time to execute 
...
Two changes were made. The first was \textit{QuantumMode}, and the other was the inclusion of more simulation 
...
First of all, the \textit{QuantumMode} was defined and added to the list of the simulator's available types. After 
that, a new file was created to implement the name translation, due to the fact that the names used on the user side are 
different from the simulator side. At last, the remaining simulator was adapted to be able to select between the two versions, 
